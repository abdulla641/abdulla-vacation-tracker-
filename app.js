...
function setMarriageFilter(filter,btn){
  marriageFilter=filter;document.querySelectorAll('.chip').forEach(x=>x.classList.remove('active'));if(btn)btn.classList.add('active');renderMarriageHistory();
}

function render(){
  const av=wallet.reduce((a,b)=>a+Number(b.amount||0),0);
  const sp=expenses.reduce((a,b)=>a+Number(b.amount||0),0);
  ...
  renderTripDay();renderMarriage();renderReports();
}
...
function renderMarriage(){
  ...
  const pct=budget?Math.min(100,(spent/budget)*100):0;const remaining=budget-spent;
  ['marriageBudget'].forEach(id=>qs(id).textContent=money(budget));
  qs('marriageSpent').textContent=money(spent);qs('marriageFunded').textContent=money(funded);qs('marriageBorrowed').textContent=money(borrowed);qs('marriagePercent').textContent=(spent/budget*100||0).toFixed(spent?1:0)+'%';qs('marriageRemaining').textContent=money(remaining);
  qs('marriageProgress').style.width=pct+'%';qs('marriageMiniProgress').style.width=pct+'%';qs('marriageBudgetSummary').textContent=`${money(spent)} / ${money(budget)}`;qs('marriageBudgetInput').value=budget;qs('marriageStage').value=marriageSettings.stage;
  ...
  renderMarriageCategories();renderMarriageHistory();renderReports();
}
...
function renderReports(){
  ...
  const budget=Number(marriageSettings.budget)||300000,ms=marriageExpenses.reduce((a,b)=>a+Number(b.amount||0),0),mf=marriageFunding.reduce((a,b)=>a+Number(b.amount||0),0),mb=marriageFunding.filter(x=>x.fundingType==='borrowed').reduce((a,b)=>a+Number(b.amount||0),0);
  qs('marriageReport').innerHTML=`<div class="report-grid"><div><span>Status</span><strong>${marriageSettings.stage==='confirmed'?'Confirmed':'Proposal'}</strong></div><div><span>Actual spent</span><strong>${money(ms)}</strong></div><div><span>Budget left</span><strong>${money(budget-ms)}</strong></div><div><span>Total funding</span><strong>${money(mf)}</strong></div><div><span>Borrowed</span><strong>${money(mb)}</strong></div><div><span>Unfunded spend</span><strong>${money(Math.max(0,ms-mf))}</strong></div></div>`;
}

function exportData(){
  const data={exportedAt:new Date().toISOString(),vacation:{wallet,expenses},marriage:{settings:marriageSettings,expenses:marriageExpenses,funding:marriageFunding}};
